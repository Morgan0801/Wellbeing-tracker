...
export function SessionTagSelector({ value, onChange }: SessionTagSelectorProps) {
  ...
  const handleDeleteTag = async (tagId: string, isDefault: boolean) => {
    ...
    if (confirm('Êtes-vous sûr de vouloir supprimer ce tag ?')) {
      ...
      // Si le tag supprimé était sélectionné, désélectionner
      ...
      <Card className="mt-4">
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {/* Bouton "Aucune catégorie" */}
            <button
              onClick={() => onChange(null)}
              className={cn(
                "px-3 py-2 rounded-xl text-sm font-medium transition-all border-2",
                value === null
                  ? "bg-muted/20 border-foreground text-foreground shadow-soft-md"
            ...
            {tags.map((tag) => {
              ...
              return (
                <div key={tag.id} className="relative group">
                  <button
                    onClick={() => onChange(isSelected ? null : tag.name)}
                    className={cn(
          ...
          {value && (
            <p className="text-xs text-muted-foreground mt-3">
              Les sessions seront enregistrées avec la catégorie <strong>#{value}</strong>
            </p>
          )}
        </CardContent>
      </Card>
